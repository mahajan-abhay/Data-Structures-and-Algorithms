...
bool areIdentical(TreeNode<int> *root1, TreeNode<int> * root2) {
    ...
    {
        TreeNode<int> * frontelement1 = pendingnodes1.front();
        TreeNode<int> * frontelement2 = pendingnodes2.front();
        pendingnodes1.pop();
        pendingnodes2.pop();
        if(frontelement1 -> children.size() != frontelement2 -> children.size())
        {
            return false;
            break;
        }
        for(int i = 0; i< frontelement1 -> children.size() ; i++)
        {
            if((frontelement1 -> data == frontelement2 -> data) && (frontelement1 -> children[i] -> data == frontelement2 -> children[i] -> data))
            {
                answer = true;
                pendingnodes1.push(frontelement1 -> children[i]);
                pendingnodes2.push(frontelement2 -> children[i]);
            }
            else
            {
                return false;
